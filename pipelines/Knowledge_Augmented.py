...
def gen_descript(goList_file):
    ...
    for i in range(1, len(go_records)):
...
def merge_go_expert_knowledge(descriptions, go_knowledge):
    ...
    for i in trange(len(descriptions)):
        ...
        enhanced_descriptions.append({'Step': step, 'Time': descriptions[i]["Time"], 'Commentary': response, 'Short_commentary': descriptions[i]["Short_Description"]})
