...
def match_level(level_num):
    if level_num >= 0 and level_num <= 2:
        return "新手"
    elif level_num >= 3 and level_num <= 5:
        return "了解"
    elif level_num >= 6 and level_num <= 8:
        return "熟悉"
    elif level_num >= 9 and level_num <= 10:
        return "精通"
    else:
        return "新手"
...
def get_commentary_script(step, time_sep, time, docs, short_com, desc, level=2):
    ...
    You need to generate about {char_number} seconds of commentary, you don't need to say the opening remarks as well as the closing remarks, just generate it for the available information.
    ...
    commentary_script = chain.invoke({"char_number": time_sep, "level": match_level(level_num=level), "description": desc, "context": docs})
    # print(commentary_script)
    global commentary_script_json
    commentary_script_json.append({"step": step, "Time_sep": time_sep, "Time": time, "Commentary": commentary_script.content, "Short_Commentary": short_com})
...
def retrieval_aug(lev):
    ...
    with open(configs['go_expert_knowledge_description_json_path'], mode='r', encoding='utf-8') as f:
        ...
        for i in range(len(json_file)-1):
            ...
            add_history_knowladge == True if  time_sep > 30 else add_history_knowladge == False
            ...
            get_commentary_script(step=i+1, time_sep=time_sep, time=json_file[i]['Time'], docs=context, desc=description, short_com=json_file[i]['Short_commentary'], level=lev)
...
if __name__ == '__main__':
    ...
    with open(configs['go_expert_knowledge_description_json_path'], mode='r', encoding='utf-8') as f:
        ...
        for i in range(0,len(json_file)-1):
            ...
            if add_history_knowladge == True:
                retriever = vectorstore1.as_retriever(search_kwargs={"k": 1})
                context = get_docs(retriever=retriever)
            else:
                if routing_db.datasource ==  'classic_go_match':
                    retriever = vectorstore2.as_retriever(search_kwargs={"k": 2})
                    context = get_docs(retriever=retriever)
            ...
            print('####################### Finish Database Retrieval {} #######################'.format(i+1))
            get_commentary_script(step=i+1, time_sep=time_sep, time=json_file[i]['Time'], docs=context,  short_com=json_file[i]['Short_commentary'], level=2)
